...
//        driver.navigate().to("https://staging5.openquire.com/reports/666234");
        driver.navigate().to("https://staging3.openquire.com/reports/1595282");
        Thread.sleep(2000);
...
//    }
    ...
    public void register_ck4_feature() throws InterruptedException {
        // Step 1: Unregister all CK5 features first
        System.out.println("⏳ Checking for CK5 features to unregister...");
        logger.log(LogStatus.INFO, "⏳ Checking for CK5 features to unregister...");
        ...
        for (String feature : ck5Features) {
            try {
                ...
                if (unregisterButton.isDisplayed()) {
                    ...
                    driver.switchTo().alert().accept();
                    Thread.sleep(3000);
                    System.out.println("✅ " + feature + " unregistered successfully");
                }
            } catch (Exception ex) {
        ...
        // Step 2: Register CK4 AI features if not already registered
        System.out.println("⏳ Checking and registering CK4 AI features...");
        String[] ck4Features = {
                "CKEditor 4 - AI",
        ...
        for (String feature : ck4Features) {
            try {
                ...
                WebElement registeredFeature = driver.findElement(By.xpath("//tr[td[normalize-space()='" + feature + "']]"));
                if (registeredFeature.isDisplayed()) {
                    System.out.println("✅ " + feature + " is already registered...skipping");
                    continue;
                }
            } catch (Exception ex) {
                // Feature not registered, try to add it
                System.out.println("➕ Attempting to register: " + feature);
                try {
                    BaseClass.staging5().click_add_company_feature();
                    Thread.sleep(1000);

                    // Check if feature exists in dropdown
                    Select dropdown = new Select(companyFeatureDropdown);
                    boolean featureFound = false;
                    for (WebElement option : dropdown.getOptions()) {
                        if (option.getText().equals(feature)) {
                            featureFound = true;
                            break;
                        }
                    }
                    ...
                    if (featureFound) {
                        ...
                        Thread.sleep(1000);
                        System.out.println("✅ " + feature + " registered successfully");
                    } else {
                        System.out.println("⚠️ " + feature + " not found in dropdown...closing modal");
                        // Close modal without saving
                        WebElement cancelButton = driver.findElement(By.xpath("//a[contains(text(),'Cancel')] | //button[contains(text(),'Cancel')] | //a[@class='close']"));
                        cancelButton.click();
                        Thread.sleep(500);
                    }
                } catch (Exception regEx) {
                    System.out.println("❌ Failed to register " + feature + ": " + regEx.getMessage());
        ...
        System.out.println("✅ CK4 AI features registered successfully");
        logger.log(LogStatus.PASS, "✅ CK4 AI features registered successfully");
    }
    @FindBy(xpath = "//tr[td[normalize-space()='CKEditor 4 - AI']]//a[contains(text(),'unregister')]")
    WebElement unregister_ck4_ai_feature;

    public void scroll_and_click_unregister_ck4_feature() throws InterruptedException {
        try
        {
            if (unregister_ck4_ai_feature.isDisplayed()) {
                String[] ck4Features = {
                        "CKEditor 4 - AI",
                        "CKEditor 4 - AI Assistant",
                        "CKEditor 4 - AI Generative",
...
//    }
    ...
    @FindBy(xpath = "//a[normalize-space()='Signature 1']")
    ...
    @FindBy(xpath = "//a[@title='Revisions']")
    WebElement intro_pages_content_editor_revisions_icon;

    public void click_intro_pages_content_editor_revisions_icon() {
        ReusableMethodsLoggersPOM.clickMethod(driver, intro_pages_content_editor_revisions_icon, logger, "intro_pages_content_editor_revisions_icon");
    }
...
// Assert all
    ...
    @FindBy(xpath = "//a[normalize-space()='Quire']")
    WebElement quireLink;

    @FindBy(xpath = "//a[normalize-space()='QATE']")
    WebElement qateWidgetLink;

    public void click_quire_then_qate_company_widget() throws InterruptedException {
        ReusableMethodsLoggersPOM.clickMethod(driver, quireLink, logger, "quireLink");
        Thread.sleep(1000);
        ReusableMethodsLoggersPOM.clickMethod(driver, qateWidgetLink, logger, "qateWidgetLink");
    }

/* <<<<<<<<<<  0b56e8b6-436f-40f8-ae9e-c82720bb5bb0  >>>>>>>>>>> */
//    @FindBy(xpath = "(//table[@class='htCore'])//tr//td[2]")
//    List<WebElement> pca_portfolio_report_name_cells;
//     List<String> tableTextList = new ArrayList<>();
